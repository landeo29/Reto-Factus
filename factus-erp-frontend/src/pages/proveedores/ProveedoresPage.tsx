...
export default function ProveedoresPage() {
    ...
    const loadProveedores = async () => {
        try {
            const res = await api.get('/erp/proveedores');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (editing) {
                await api.put(`/erp/proveedores/${editing.id}`, form);
                toast.success('Proveedor actualizado');
            } else {
                await api.post('/erp/proveedores', form);
            ...
            toast.error(err.response?.data?.message || 'Error guardando proveedor');
        }
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await api.delete(`/erp/proveedores/${id}`);
    ...
    const openCreate = () => {
        setEditing(null);
        setForm({ tipoIdentificacion: 'NIT', numeroIdentificacion: '', dv: '', nombres: '', direccion: '', email: '', telefono: '', personaContacto: '' });
        setShowModal(true);
    };
    ...
    const filtered = proveedores.filter(p =>
        ...
        p.numeroIdentificacion.includes(search)
    );
        ...
        <div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        ...
                        <tr className="border-b border-gray-200 bg-gray-50/50">
                            ...
                            <th className="text-left text-[11px] font-bold text-gray-500 uppercase tracking-wider px-5 py-3">Contacto</th>
                        ...
                        {loading ? (
                            ...
                            <tr><td colSpan={6} className="text-center py-10 text-sm text-gray-400">No hay proveedores registrados</td></tr>
                        ) : (
                            filtered.map((p, i) => (
                                <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors animate-fade-up" style={{ animationDelay: `${i * 40}ms` }}>
                                    <td className="px-5 py-3.5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-secondary/10 flex items-center justify-center text-secondary text-xs font-bold">
                                                {p.nombres.charAt(0)}
                                            </div>
                                            <span className="text-sm font-semibold text-dark">{p.nombres}</span>
                                        </div>
                                    </td>
                                    <td className="px-5 py-3.5">
                                        <span className="text-xs font-mono text-gray-500">{p.tipoIdentificacion} {p.numeroIdentificacion}</span>
                                    </td>
                                    <td className="px-5 py-3.5 text-xs text-gray-500">{p.personaContacto || '—'}</td>
            ...
            <Modal open={showModal} onClose={closeModal}>
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/80">
                    <div>
                        <h2 className="text-sm font-bold text-dark">{editing ? 'Editar Proveedor' : 'Nuevo Proveedor'}</h2>
                        <p className="text-[11px] text-gray-400 mt-0.5">{editing ? 'Modifica los datos del proveedor' : 'Completa los datos para registrar'}</p>
                    </div>
                    ...
                    <div className="p-6 space-y-5">
                        <div>
                            ...
                            <div className="grid grid-cols-5 gap-3">
                                ...
                                <div className="col-span-3">
                                    ...
                                    <input type="text" value={form.numeroIdentificacion} onChange={e => setForm({...form, numeroIdentificacion: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="900123456" required disabled={!!editing} />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Razón Social / Nombre</label>
                                    <input type="text" value={form.nombres} onChange={e => setForm({...form, nombres: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="Distribuidora XYZ S.A.S" required />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Persona de Contacto</label>
                                    <input type="text" value={form.personaContacto} onChange={e => setForm({...form, personaContacto: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="María González" />
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 mb-1.5">Email</label>
                                        <input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="proveedor@empresa.com" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 mb-1.5">Teléfono</label>
                                        <input type="text" value={form.telefono} onChange={e => setForm({...form, telefono: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="601 234 5678" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-gray-600 mb-1.5">Dirección</label>
                                    <input type="text" value={form.direccion} onChange={e => setForm({...form, direccion: e.target.value})} className="w-full px-3 py-2.5 bg-white border border-gray-200 rounded-xl text-sm text-dark placeholder-gray-300 focus:border-primary focus:ring-4 focus:ring-primary-50 outline-none transition-all" placeholder="Carrera 7 #45-12, Bogotá" />
                                </div>
                            </div>
                        </div>
                    </div>
